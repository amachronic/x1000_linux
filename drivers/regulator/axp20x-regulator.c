...
struct axp20x_regulator_data {
	 ...
	 * regulator subsystem. Unsupported regulators will be ignored.
	 */
	bool (*is_regulator_supported)(struct regmap *regmap, int id);
...
static int axp20x_regulator_set_ramp_delay(struct regulator_dev *rdev, int ramp)
{
	...
	if (!data->set_ramp_delay)
		return -ENOTSUPP;

	return data->set_ramp_delay(rdev, ramp);
}
...
static int axp20x_regulator_enable_regmap(struct regulator_dev *rdev)
{
	...
	/*
	 * Check if we support emulated soft start for this regulator.
	 */
	if (rdev->constraints && rdev->constraints->soft_start &&
	    data->has_soft_start_emulation &&
	    data->has_soft_start_emulation(id)) {
		int v_out;
		int ret;
...
static bool axp803_is_regulator_supported(struct regmap *regmap, int id)
{
	unsigned int reg;
...
static bool axp806_is_regulator_supported(struct regmap *regmap, int id)
{
	unsigned int reg;
...
static bool axp813_is_regulator_supported(struct regmap *regmap, int id)
{
	/* Support for FLDO3 is not implemented */
	if (id == AXP813_FLDO3)
		return false;

	/* Otherwise this is the same as the AXP803 */
	return axp803_is_regulator_supported(regmap, id);
}
...
static int axp20x_regulator_probe(struct platform_device *pdev)
{
	 ...
	 */
	rdevs = kcalloc(data->num_regulators, sizeof(*rdevs), GFP_KERNEL);
	if (!rdevs)
		return -ENOMEM;
	...
	for (i = 0; i < data->num_regulators; i++) {
		...
		/* Skip unsupported regulators */
		if (data->is_regulator_supported &&
		    !data->is_regulator_supported(axp20x->regmap, i))
			continue;
		...
		if (!desc->supply_name) {
			...
			if (data->get_supply_regulator_id)
				supply_id = data->get_supply_regulator_id(desc->id);
		...
		ret = of_property_read_u32(rdev->dev.of_node,
					   ...
					   &workmode);
		if (!ret) {
			if (data->set_dcdc_workmode ||
			    data->set_dcdc_workmode(rdev, i, workmode))
